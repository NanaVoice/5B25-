...
public class Cam : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        targetPos = new Vector3(m_playerTransform.position.x, m_playerTransform.transform.position.y+1.0f, gameObject.transform.position.z);

        if (m_playerTransform.position.x > 0f)
        {
            targetPos = new Vector3(m_playerTransform.position.x + Ahead, m_playerTransform.transform.position.y + 1.0f, gameObject.transform.position.z);
        }
        else
        {
            targetPos = new Vector3(m_playerTransform.position.x - Ahead, m_playerTransform.transform.position.y + 1.0f, gameObject.transform.position.z);
